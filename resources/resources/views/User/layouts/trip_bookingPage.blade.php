...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}"> 
    <head>
        ...
        <title>{{ $data -> trip_name }}</title>
        <meta name="description" content="{{ $data -> other_details }}" />
        <link rel="canonical" href="{{url()->full()}}" />
    	<meta property="og:locale" content="en_US" />
    	<meta property="og:type" content="website" />
    	<meta property="og:title" content="{{ $data -> trip_name }}" />
    	<meta property="og:description" content="Book {{ $data -> trip_name }}. Visit our website to customise your dream destination" />
...
{"@type":"WebPage",
		"@id":"{{url()->full()}}#webpage",
			"url":"{{url()->full()}}",
			"name":"{{ $data -> trip_name }} for {{$nod}} days and {{$nop}} persons",
			...
			"author":{"@id":"https://touristtribe.in/{{ $data -> location_name }}"},
			"description":"{{ $data -> other_details }}",
...
{"@type":"BreadcrumbList",
			...
			"position":2,
			"name":"{{ $data -> trip_name }} for {{$nod}} days and {{$nop}} persons"}]},												
			{"@type":"Person","@id":"https://touristtribe.in/{{ $data -> location_name }}",
			"name":"{{ $data -> trip_name }} for {{$nod}} days and {{$nop}} persons",
			...
			"caption":"{{ $data -> trip_name }} for {{$nod}} days and {{$nop}} persons"},
			"sameAs":["https://touristtribe.in/"],
			"url":"https://touristtribe.in/{{ $data -> location_name}}"}]}
     
      
      
      "@type": "NewsArticle",
      "headline": "{{ $data -> trip_name }} for {{$nod}} days and {{$nop}} persons",
      "image": [
    ...
    <script type="application/ld+json">
    { "@context": "https://schema.org/", "@type": "Product", "name": "{{ $data -> trip_name }} for {{$nod}} days and {{$nop}} persons", "image": [ "{{ secure_asset('storage/'.$midtrips->images) }}" ], "description": "Hotel: {{ $hotels -> rating }} Star, Transfer: {{ $data -> vehicalName }}, {{ $data -> other_details }}", "sku": "0446310786", "mpn": "925872", "brand": { "@type": "Brand", "name": "TouristTribe" }, "review": { "@type": "Review", "reviewRating": { "@type": "Rating", "ratingValue": "4", "bestRating": "5" }, "author": { "@type": "Person", "name": "Khalid Hossain" } }, "aggregateRating": { "@type": "AggregateRating", "ratingValue": "4.4", "reviewCount": "89" }, "offers": { "@type": "Offer", "url": "{{url()->full()}}", "priceCurrency": "INR", "price": "{{ base64_decode($trip_price) }}", "priceValidUntil": "{{ date('Y-m-d', strtotime('+3 Months')) }}", "itemCondition": "https://schema.org/NewCondition", "availability": "https://schema.org/InStock" } } </script>            
        <!-- Google Font -->
