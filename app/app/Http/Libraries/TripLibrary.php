...
class TripLibrary{
    public static function get_trip_calculated_details($loc, $nod, $nop, $nor, $bt, $trip_date, $trip_id = null, $params = []){
        // $params["commission_percent"] = 0.1;
        
        $commission_percent = ($params && array_key_exists("commission_percent", $params) && $params["commission_percent"]) ? 1 + $params["commission_percent"] : 0;
        ...
        $db = DB::table('trips')
                ...
                ->select
                    (
                        ...
                        'trips.id','trips.trip_name', 'trips.location_name','trips.no_of_days','trips.no_of_nights',
                        ...
                        'trips.christianCast','trips.hotels','trips.midtrips', 'trips.starting_date', 'trips.ending_date','meals.per_head_cost as mealPrice',
        ...
        // if($trip_date){
        //     $db->where('trips.strating_date', $trip_date);
        // }
        if($nod){
            $db->where('trips.no_of_days',$nod);
        }
        if($loc){
            $db->where('trips.location_name',$loc);
        }
        $data = $db->get();
        ...
        foreach ($data as $datas) {
            if($data != '[]'){
               ...
               // $license_fee_amount = $nop*$datas->per_head_cost;
                $license_fee_amount = $datas->per_head_cost;
                ...
                $tripPrices = 0;
                $tour_operator_percent_amount = 0;
                $payable_amount = 0;
                $subtotal_price = $hotel_price +  $p + $mealPrice + $license_fee_amount;
                $tripPrices = $subtotal_price;
                
                //Touristtribe 
                $tourist_percent_price = $tripPrices * 0.08;//
                ...
                if($commission_percent){
                    $tour_operator_percent_amount = $tripPrices * $commission_percent;//tour_operator_percent_amount 
                    $tripPrices = $tripPrices + $tour_operator_percent_amount;
                }

                // echo $tripPrices." ".$tourist_percent_price." ".$tour_operator_percent_amount;die;
                $tripPrices = (int)$tripPrices;


                $tripPricesData[$incrementData] = [
                    "hotel_price" => $hotel_price,
                    "transfer_price" => $p,
                    "license_fees_amount" => $license_fee_amount,
                    "meal_price" => $mealPrice,
                    "subtotal_price" => $subtotal_price,
                    "tourist_percent_price" => $tourist_percent_price,
                    "tour_operator_percent_amount" => $tour_operator_percent_amount,
                    "gst_price" => $gst_price,
