...
class UserActionsControllers extends Controller
{
    public function signup(Request $req){
        ...
        if($password == $confirm_password){
            ...
            Mail::to($req->input('email_id'))->send(new \App\Mail\sendMailOtp($otp));
            
            $message = TextLocal::get_message_template("seller_registration_otp", ['otp' => $otp]);
    ...
    public function signup_seller(Request $request){
        ...
        // echo TourOperator::get_camel_case("The New User 2");die;
        // echo 1;die;
        if($request->isMethod('post')){
            try{
                ...
                $validator = Validator::make($request->all(), [
                    ...
                    'company_name' => 'required|unique:tour_operators,comp_name',
                    'adhar_number' => 'required|digits:12',
                    'gst' => 'required',
                    'password' => 'required|min:6|max:20',
                    //'confirm_password' => 'required',
                    // 'confirm_password' => 'required|min:6|regex:/^.*(?=.{3,})(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!$#%]).*$/ |max:20|confirmed',
                ]);
                ...
                $comp_name_slug = TourOperator::get_comp_name_slug($company_name);
                ...
                if(in_array(strtolower($company_name), ["admin", "seller"])) throw new Exception("Company name is invalid.");
    ...
    //             TextLocal::send_message($mobile, $message);
                ...
                $tour_operator->comp_name_slug = $comp_name_slug;
    ...
    public function login_seller(Request $request){
        ...
        if($request->isMethod('post')){
            try{
                ...
                if(!$seller->verify) return redirect()->route('verification_seller');

                if(!$seller->status) throw new Exception('You are inactive, Please contact to Admin.');
    ...
    public function forget_password_otp_verification(Request $request){
		...
		if($request->isMethod('post')){
			try{
				...
				$validator = Validator::make($request->all(), [
					'otp' => 'required|numeric|digits:6',
					'new_password' => 'required|min:6|regex:/^.*(?=.{3,})(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!$#%]).*$/ |max:20',
					'confirm_new_password' => 'required|min:6|regex:/^.*(?=.{3,})(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!$#%]).*$/ |max:20|confirmed',
				]);
