...
class TripControllers extends Controller
{
    ...
    public function send(Request $req){
        ...
        // License Fee Details
        $license_fee_details = PerHeadExtraFee::where('location_id', $location_id)->get();
        ...
        $data = [
        		...
        		"location_id"=>$location_id,"hotel"=>$hotels,"license_fee_details"=>$license_fee_details,
    ...
    public function edit_trip(Request $request, $trip_id){
        $data = [];
        $data["error"] = "";
        $data["message"] = "";
        ...
        if($request->isMethod('post')){
            try{
                ...
                $trip_name = trim($request->input('trip_name'));
                $pickup = trim($request->input('pickup'));
                $drop = trim($request->input('drop'));
                $details = trim($request->input('detail'));

                Trip::where('id', $trip_id)->update([
                    'trip_name' => $trip_name ? $trip_name : null,
                    'pickup_details' => $pickup ? $pickup : null,
                    'drop_details' => $drop ? $drop : null,
                    'other_details' => $details ? $details : null
                ]);

                $data["message"] = "Successfully Updated";
                return redirect()->route('seller_trip');
        ...
        return view('Seller.edit_trip', $data);
    }
}
